...
isAnimate = False
control_points = []
catmull_coefficient1 = np.array([
    [2, -2, 1, 1],
    [-3, 3, -2, -1],
    [0, 0, 1, 0],
    [1, 0, 0, 0]
])
catmull_coefficient2 = np.array([
    [0, 1, 0, 0],
    [0, 0, 1, 0],
    [-.5, 0, .5, 0],
    [0, -.5, 0, .5]
...
q_arr = []
catmull = catmull_coefficient1 @ catmull_coefficient2
move_cow = None
index = 0
animStartTime = 0
push = -1
...
def display():
    global cameraIndex, cow2wld, isAnimate, control_points, move_cow, catmull, index, animStartTime, isCopyStart, push, q_arr
    ...
    if(isAnimate == True):
        animTime = glfw.get_time() - animStartTime
        t = np.array([animTime * animTime * animTime, animTime * animTime, animTime, 1])
        next_point = t @ catmull @ q_arr

        # rotate cow
        x_axis = normalize(next_point - getTranslation(move_cow))
        z_axis = normalize(np.cross(x_axis, vector3(0, 1, 0)))
        y_axis = normalize(np.cross(z_axis, x_axis))

        move_cow[0:3, 0] = x_axis
        move_cow[0:3, 1] = y_axis
        move_cow[0:3, 2] = z_axis

        # move cow
        if (animTime > 1):
            animStartTime = animStartTime + animTime
            index += 1
            ...
            q_arr = np.array([
                getTranslation(control_points[input]),
                getTranslation(control_points[(index % 6)]),
                getTranslation(control_points[(index % 6) + 1]),
                getTranslation(control_points[(index % 6) + 2])
            ])

            if (index == (len(control_points) - 2) * 3):
                isAnimate = False
                control_points = []
                index = 0
                animStartTime = 0
                ...
                move_cow[0:3, 0] = vector3(1,0,0)
                move_cow[0:3, 1] = vector3(0,1,0)
                move_cow[0:3, 2] = vector3(0,0,1)

        setTranslation(move_cow, next_point)
        drawCow(move_cow, cursorOnCowBoundingBox)
    else:
        if(isCopyStart):
            drawCow(cow2wld, cursorOnCowBoundingBox)
            for i in range(len(control_points)):
                drawCow(control_points[i], cursorOnCowBoundingBox)		# Draw cow.
        else :
            drawCow(cow2wld, cursorOnCowBoundingBox)
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, isAnimate, control_points, move_cow, animStartTime, push, isCopyStart, q_arr
    GLFW_DOWN=1;
    GLFW_UP=0;
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            ...
            if(push > -1 and isAnimate == False):
                control_point_now = copy.copy(cow2wld)
                control_points.append(control_point_now)
                if(len(control_points) == 6):
                    isAnimate = True

                    q_arr = np.array([
                        getTranslation(control_points[5]),
                        getTranslation(control_points[0]),
                        getTranslation(control_points[1]),
                        getTranslation(control_points[2])
                    ])
    ...
    if isDrag != 0:
        ...
        if  isDrag==V_DRAG:
            ...
            if cursorOnCowBoundingBox:
                ...
                print("in %d drag (%d, %d)" % (isDrag, getTranslation(cow2wld)[0], getTranslation(cow2wld)[1]))
...
def main():
    ...
    width = 800;
    height = 600;
    window = glfw.create_window(width, height, '2017029925-PA2', None, None)
    if not window:
        glfw.terminate()
        sys.exit(-1)
